...
def _carla_schema_guard(envelope: TransitEnvelope) -> bool:
    ...
    return (type(missing_data_count) is int) and missing_data_count >= 0
...
def _carla_health_guard(envelope: TransitEnvelope) -> bool:
    ...
    if lidar_enabled:
        ...
        if lidar_natural_dropped is not None and (
            not (type(lidar_natural_dropped) is int) or lidar_natural_dropped != 0
        ):
            return False

        lidar_injected_dropped = iface.get("lidar_frames_injected_dropped")
        if lidar_injected_dropped is not None and (
            not (type(lidar_injected_dropped) is int) or lidar_injected_dropped != 0
        ):
            return False
