...
def _carla_schema_guard(envelope: TransitEnvelope) -> bool:
    ...
    if type(ticks_requested) is not int or ticks_requested <= 0:
        return False

    ticks_completed = iface.get("ticks_completed")
    if type(ticks_completed) is not int or ticks_completed < 0:
        return False
...
def _carla_health_guard(envelope: TransitEnvelope) -> bool:
    ...
    if lidar_enabled:
        ...
        if type(lidar_frames) is not int or lidar_frames <= 0:
            return False
        ...
        if type(lidar_dropped) is not int or lidar_dropped != 0:
            return False

        lidar_cb_errors = iface.get("lidar_callback_errors")
        if type(lidar_cb_errors) is not int or lidar_cb_errors != 0:
            return False
