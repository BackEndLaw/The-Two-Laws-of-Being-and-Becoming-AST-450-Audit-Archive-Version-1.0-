...
class LidarCollector:
    ...
    ``fault_notify`` is an optional callable invoked once on the sensor thread
    when fault injection triggers.  It receives ``(callback_index, sensor_frame)``
    and MUST NOT call any CARLA vehicle control API.  It is called after
